...
namespace inflation {
    ...
    std::vector<DataType> parameter_creator(DataType Vzero, DataType A) {
        std::vector<DataType> output{Vzero, A};
        return output;
    }
    ...
    int large_epsilon = 0;
    int neg_V = 0;
    int failed_horizonExit = 0;
    int ics_before_start = 0;
    int inflate60 = 0;
    int time_var_pow_spec = 0;
...
//        fNL_squ_piv = sq_redbsp_samples.back();
    ...
    status = block->put_val( inflation::fail_names, "CppT_runt_excpt",    inflation::runtime_exception);
