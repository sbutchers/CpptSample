...
namespace inflation {
    ...
    DataType M_P, lambda, phi_init, phiDot_init;
    ...
    std::vector<DataType> parameter_creator(DataType lambda) {
        std::vector<DataType> output{lambda};
        return output; 
    }

    // function to create a std::vector<DataType> vector containing initial conditons
    std::vector<DataType> init_cond_creator(DataType phi_init, DataType phiDot_init) {
        std::vector<DataType> output{phi_init, phiDot_init};
        return output;
    }
...
static std::unique_ptr< transport::quartic_mpi<DataType, std::vector<DataType>> > model;
...
void * setup(cosmosis::DataBlock * options)
{
    ...
    model = std::make_unique< transport::quartic_mpi<DataType, std::vector<DataType>> > (env, arg);
...
DATABLOCK_STATUS execute(cosmosis::DataBlock * block, void * config)
{
    ...
    block->get_val(inflation::paramsSection, "lambda", inflation::lambda);
    ...
    transport::parameters<DataType> params{inflation::M_P, inflation::parameter_creator(inflation::lambda), model.get()};
    transport::initial_conditions<DataType> ics{"quartic", params, inflation::init_cond_creator(inflation::phi_init,
    ...
    try {
        ...
        transport::twopf_task<DataType> tk2_test{"quartic.twopf_test", ics, times, k_test};
...
//        }
        ...
        tk2 = std::make_unique< transport::twopf_task<DataType> > ("quartic.twopf", ics, times_sample, ks);
        tk2->set_adaptive_ics_efolds(4.5);
        // construct a twopf task for the pivot scale
        tk2_piv = std::make_unique< transport::twopf_task<DataType> > ("quartic.twopf-pivot", ics, times_sample, k_pivot_range);
        tk2_piv->set_adaptive_ics_efolds(4.5);
        // construct an equilateral threepf task based on the kt pivot scale made above
        tk3e = std::make_unique< transport::threepf_alphabeta_task<DataType> > ("quartic.threepf-equilateral", ics,
        ...
        tk3s = std::make_unique< transport::threepf_alphabeta_task<DataType> > ("quartic.threepf-squeezed", ics,
