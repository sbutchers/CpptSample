...
namespace inflation {
    ...
    double M_P, lambda, phi_init, phiDot_init;
...
DATABLOCK_STATUS execute(cosmosis::DataBlock * block, void * config)
{
    ...
    transport::parameters<double> params{inflation::M_P, inflation::parameter_creator(inflation::lambda), model.get()};
    transport::initial_conditions<double> ics{"quartic", params, inflation::init_cond_creator(inflation::phi_init,
    ...
    try {
        ...
        transport::twopf_task<double> tk2_test{"quartic.twopf_test", ics, times, k_test};
...
//        }
        ...
        tk2 = std::make_unique< transport::twopf_task<double> > ("quartic.twopf", ics, times_sample, ks);
        tk2->set_adaptive_ics_efolds(4.5);
        // construct a twopf task for the pivot scale
        tk2_piv = std::make_unique< transport::twopf_task<double> > ("quartic.twopf-pivot", ics, times_sample, k_pivot_range);
        tk2_piv->set_adaptive_ics_efolds(4.5);
        // construct an equilateral threepf task based on the kt pivot scale made above
        tk3e = std::make_unique< transport::threepf_alphabeta_task<double> > ("quartic.threepf-equilateral", ics,
        ...
        tk3s = std::make_unique< transport::threepf_alphabeta_task<double> > ("quartic.threepf-squeezed", ics,
