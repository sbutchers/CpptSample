...
namespace inflation {
    ...
    DataType M_P, V0, eta_R, g_R, lambda_R, alpha, R0, R_init, theta_init, Rdot_init, thetadot_init;
    ...
    std::vector<DataType> parameter_creator(DataType R0, DataType V0, DataType eta_R, DataType g_R, DataType lambda_R, DataType alpha) {
        std::vector<DataType> output{R0, V0, eta_R, g_R, lambda_R, alpha};
        return output; 
    }

    // function to create a std::vector<DataType> vector containing initial conditons
    std::vector<DataType> init_cond_creator(DataType R_init, DataType theta_init, DataType Rdot_init, DataType thetadot_init) {
        std::vector<DataType> output{R_init, theta_init, Rdot_init, thetadot_init};
        return output;
    }
...
static std::unique_ptr< transport::gelaton_mpi<DataType, std::vector<DataType>> > model;
...
void * setup(cosmosis::DataBlock * options)
{
    ...
    model = std::make_unique< transport::gelaton_mpi<DataType, std::vector<DataType>> > (env, arg);
...
//    std::cout << "thetadot_init = " << inflation::thetadot_init << std::endl;
    ...
    transport::parameters<DataType> params{inflation::M_P, inflation::parameter_creator(inflation::R0, inflation::V0,
            inflation::eta_R, inflation::g_R, inflation::lambda_R, inflation::alpha), model.get()};
    transport::initial_conditions<DataType> ics{"gelaton", params, inflation::init_cond_creator(inflation::R_init,
